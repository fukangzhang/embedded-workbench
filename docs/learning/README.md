...
- `2026-05-09-STM32RCCUSART时钟.md`
- `2026-05-09-STM32USART寄存器Helper.md`
- `2026-05-09-STM32USART串口IO适配器.md`
...
- `stm32_usart_serial_io` 如何把 USART 单字节读写包装成后续串口命令服务可用的 callback
...
- `2026-05-08-C代码注释阅读方法.md`

然后按“头文件 -> 源文件 -> 测试”的顺序读代码。学习笔记负责解释背景和路线，代码内注释负责解释你正在看的那几行为什么这样写。

遇到 `指针`、`volatile`、`位运算`、`FreeRTOS 句柄`、`STM32 地址` 这些点时，优先看旁边注释；如果还是不清楚，再回到对应学习笔记找整段背景。
